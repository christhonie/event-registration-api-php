...
**source_category** | [**\Idealogic\RegistrationAPI\Model\CategoryDTO**](CategoryDTO.md) | Effective sourceCategory — the organisational Category template. On an event-level row this cascades from seriesCategory.sourceCategory if not set locally. The parentage rule forbids setting both sourceCategory and seriesCategory on the same event-level row. | [optional]
